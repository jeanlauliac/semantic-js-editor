...
export default function tokenizeBinaryOp(node, context, tokenize) {
  ...
  return [
    ...
    new Token(' ', node, TokenType.WHITE_SPACE, insertLeftNode),
    new Token(opString, node, TokenType.OPERATOR, replaceOperator),
    new Token(' ', node, TokenType.WHITE_SPACE, insertRightNode),
...
function insertLeftNode(node, character, index) {
  if (node.left !== null) {
    return node
  }
...
function insertRightNode(node, character, index) {
  if (node.right !== null) {
    return node
  }
