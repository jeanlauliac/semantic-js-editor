...
export default class Token extends Immutable.Record({
  ...
  insertChar: null,
  /**
   * `Node` that generated this token in the first place.
   */
  node: null,
  /**
   * Function that, given a position, will return a new node resulting of the
  ...
  constructor(content, node, type, insertChar, removeChar) {
    invariant(typeof content === 'string', '`content` must be a string')
    invariant(content.length > 0, '`content` cannot be empty')
    invariant(Node.TYPE.validate(node) == null, '`node` must be a `Node`')
    ...
    super({content, node, type, insertChar, removeChar})
